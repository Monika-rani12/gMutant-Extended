...
def main (input_file, output_file ) :
	...
	covered_code = open("CoveredLines.txt").read().split('\n')
...
#	ano_log_file = open(output_file+"/Orig_log_file.txt", "w") 
	...
	mutant_file=output_file+"/Meta.c"
	...
	for i in range(0,number_of_lines_of_code) :
		...
		if ((any(str(i+1) in ext for ext in covered_code)) and ("if(" in source_code[i] or "if (" in source_code[i] or "for(" in source_code[i] or "while (" in source_code[i] or "while(" in source_code[i])):
#			source_code[i]=source_code[i].replace(" ", "")
			p_n=p_n+1
			flag_p=1
		else:
			flag_p=0
...
#			print(mutated_line.index("!"))
			write_to_file(source_code, i, mutated_line)
#			print(mutated_line)
			...
			mutated_line_combined = mutated_line_combined + mutated_line + "printf(\"PNF KILLED at %d x %d,y %d,z %d \\n\"" +",__LINE__,X,Y,Z);}" + "\n"
...
#				print(mutated_line)
				...
				mutated_line_combined = mutated_line_combined + mutated_line+ "printf(\"CNF KILLED at %d x %d,y %d,z %d \\n\"" +",__LINE__,X,Y,Z);}" + "\n"
#				mutant_file=output_file+"/meta_program.c"
...
#							print(mutated_line)
							...
							if "{" not in source_code[i]:
								...
								mutated_line = mutated_line[0:ind3+1]+" ("+mutated_line[ind3+1:-1]+" != "+ rhs_predicate+"){"
							mutated_line_combined = mutated_line_combined + mutated_line + "printf(\""+rep_op+" KILLED at %d x %d,y %d,z %d \\n\"" +",__LINE__,X,Y,Z);}" + "\n"
...
mutant_count=0
def write_to_file (source_code, mutated_line_number, mutated_line ) :
#
	global mutant_count
	mutant_count=mutant_count+1
	if mutant_count == 1 :
		os.system("mkdir "+"Multiple_Mutants")
	mutant_file_name="mutant_file_"+str(mutant_count)+".c"	
	output_file = open("Multiple_Mutants/"+mutant_file_name, "w")
